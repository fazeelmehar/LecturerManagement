...
{
    // This was taken from a previous exploration in to Mediatr
    // When I was trying to learn more about CQRS in general.
         ...
         where TRequest : IRequest<TResponse>
    {
        ...
        public virtual async Task<TResponse> Handle(TRequest request, CancellationToken cancellationToken)
        {
            try
            {
                Logger.LogTrace("Processing new request '{request}' ...", request);
                var watch = Stopwatch.StartNew();
                var response = await ProcessAsync(request, cancellationToken).ConfigureAwait(false);
                watch.Stop();
                Logger.LogTrace("Request complete '{requestName}': {elapsed} ms", request, watch.ElapsedMilliseconds);
                return response;
            }
